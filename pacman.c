 ...
 * P A C M A N
 * Por:
 * Diego AndrÃ©s Alvarez MarÃ­n (daalvarez@unal.edu.co)
...
typedef struct
{
    int fil,col;              // posiciÃ³n actual (fila, columna)
    direccion dir;            // direcciÃ³n hacia la que avanza
...
struct
{
    ...
    bool invencible;          // se comiÃ³ la pepita de la invencibilidad
...
int main(void)
{
    ...
    bool salirse = false;             // Â¿salirse del juego?
    direccion proxima_dir = quieto;   // quÃ©dese quieto al principio del juego

    // Inicializar el cronÃ³metro del pacman y de los fantasmas
    ...
    while ((pacman.vidas > 0) && (!salirse))
    {
        ...
        {
            // si se puede mover en la proxima direcciÃ³n hÃ¡galo, de lo contrario
            // siga la inercia
        ...
        // vaciamos el buffer del teclado, para que lea solo lo Ãºltimo
        // especificado: esto se hace porque si se presione siempre las flechas
        // no se reaccionarÃ­a inmediatamente a la Ãºltima flecha
        flushinp();

        // pausa por el nÃºmero de milisegundos especificados
        // delay_output(1); -> lo reemplacÃ© con un timeout(1);
    ...
    for (f = 0; f < NFIL; f++)
        for (c = 0; c < NCOL; c++)
        {
            ...
            default:
                ...
                perror("Error en la especificaciÃ³n del laberinto");
                exit(EXIT_FAILURE);
            }
            // imprima el caracter + color
            mvaddch(f, c, letra | COLOR_PAIR(color));
        }
    refresh();               // imprimir en la pantalla fÃ­sica
}
...
void mover_fantasma(fantasma *F)
{
    ...
    if (num_dir_pos_mov == 1) // solo hay una posible direcciÃ³n de movimiento
    {
        // se busca y se avanza a dicha posiciÃ³n
        int i;
        for (i=1; i<=4; i++)
    ...
    // se borra el fantasma de la posiciÃ³n actual
    ...
    // se avanza el fantasma una posiciÃ³n
    F->col = col;
    F->fil = fil;

    // se pone el pacman en la nueva posiciÃ³n
...
void mover_pacman(direccion dir)
{
    ...
    // se escribe el pacman en la nueva posiciÃ³n
